...
with tab1:
    with st.sidebar:
        ...
        with st.expander("Core Inputs"):
            st.write(f"Fetched Current Price: ${float(data.get('current_price', 60000.0)):.2f} (Kraken, expect $60,000-$65,000)")
            current_price = st.number_input("Current Price (USD)", min_value=0.01, value=float(data.get('current_price', 60000.0)) if use_fetched_data else 60000.0, help="Current BTC price in USD (Kraken)")
            st.write(f"Fetched Total Supply: {float(data.get('total_supply', 21000000.0)):.0f} BTC (Fixed at 21M)")
            total_supply = st.number_input("Total Supply (BTC)", min_value=0.0, value=float(data.get('total_supply', 21000000.0)) if use_fetched_data else 21000000.0, help="Maximum BTC supply (21M)")
            st.write(f"Fetched Circulating Supply: {float(data.get('circulating_supply', 19700000.0)):.0f} BTC (Blockchain.com, expect 19.5M-20M)")
            circulating_supply = st.number_input("Circulating Supply (BTC)", min_value=0.0, value=float(data.get('circulating_supply', 19700000.0)) if use_fetched_data else 19700000.0, help="Current circulating BTC (Blockchain.com)")
            st.write(f"Fetched Next Halving Date: {data.get('next_halving_date', datetime(2028, 4, 1)).strftime('%Y-%m-%d')} (Estimate)")
            next_halving_date = st.date_input("Next Halving Date", value=data.get('next_halving_date', datetime(2028, 4, 1)) if use_fetched_data else datetime(2028, 4, 1), help="Estimated date of next halving (~April 2028)")
            st.write(f"Fetched Margin of Safety: {float(data.get('margin_of_safety', 25.0)):.2f}% (Default)")
        ...
        with st.expander("On-Chain Inputs"):
            st.write(f"Fetched Hash Rate: {float(data.get('hash_rate', 500.0)):.2f} EH/s (Blockchain.com, expect 500-600)")
            hash_rate = st.number_input("Hash Rate (EH/s)", min_value=0.0, value=float(data.get('hash_rate', 500.0)) if use_fetched_data else 500.0, help="Network hash rate (Blockchain.com)")
            st.write(f"Fetched Active Addresses: {float(data.get('active_addresses', 1000000.0)):.0f} (Blockchain.com, expect 800K-1M)")
            active_addresses = st.number_input("Active Addresses (Daily)", min_value=0.0, value=float(data.get('active_addresses', 1000000.0)) if use_fetched_data else 1000000.0, help="Daily active wallet addresses (Blockchain.com)")
            st.write(f"Fetched Transaction Volume: ${float(data.get('transaction_volume', 1e9)):.2e} (Blockchain.com, expect $1B-$5B)")
            transaction_volume = st.number_input("Transaction Volume (USD, Daily)", min_value=0.0, value=float(data.get('transaction_volume', 1e9)) if use_fetched_data else 1e9, help="Daily USD transaction volume (Blockchain.com)")
            st.write(f"Fetched MVRV: {float(data.get('mvrv', 2.0)):.2f} (Blockchain.com, expect 1.5-2.5)")
            mvrv = st.number_input("MVRV Ratio", min_value=0.0, value=float(data.get('mvrv', 2.0)) if use_fetched_data else 2.0, help="Market Value to Realized Value (Blockchain.com)")
            st.write(f"Fetched SOPR: {float(data.get('sopr', 1.0)):.2f} (Blockchain.com, expect 0.9-1.1)")
            sopr = st.number_input("SOPR", min_value=0.0, value=float(data.get('sopr', 1.0)) if use_fetched_data else 1.0, help="Spent Output Profit Ratio (Blockchain.com, ~1)")
            st.write(f"Fetched Realized Cap: ${float(data.get('realized_cap', 6e11)):.2e} (Blockchain.com, expect $600B-$800B)")
            realized_cap = st.number_input("Realized Cap (USD)", min_value=0.0, value=float(data.get('realized_cap', 6e11)) if use_fetched_data else 6e11, help="Total value of all BTC at purchase price (Blockchain.com)")
            st.write(f"Fetched Puell Multiple: {float(data.get('puell_multiple', 1.0)):.2f} (Blockchain.com, expect 0.5-1.5)")
            puell_multiple = st.number_input("Puell Multiple", min_value=0.0, value=float(data.get('puell_multiple', 1.0)) if use_fetched_data else 1.0, help="Miners' revenue vs historical avg (Blockchain.com, 0.3-5)")
            st.write(f"Fetched Electricity Cost: ${float(data.get('electricity_cost', 0.05)):.2f}/kWh (Default)")
            ...
            st.write(f"Fetched Block Reward: {float(data.get('block_reward', 6.25)):.2f} BTC (Blockchain.com, expect 3.125 post-2024 halving)")
            block_reward = st.number_input("Block Reward (BTC)", min_value=0.0, max_value=50.0, value=float(data.get('block_reward', 6.25)) if use_fetched_data else 6.25, help="Current block reward per block (Blockchain.com)")
            st.write(f"Fetched Blocks Per Day: {float(data.get('blocks_per_day', 144.0)):.0f} (Blockchain.com, expect ~144)")
            blocks_per_day = st.number_input("Blocks Per Day", min_value=100.0, max_value=200.0, value=float(data.get('blocks_per_day', 144.0)) if use_fetched_data else 144.0, help="Approx blocks mined per day (Blockchain.com)")
        
        with st.expander("Model-Specific Inputs"):
            st.write(f"Fetched S2F Intercept: {float(data.get('s2f_intercept', 14.6)):.2f} (Default)")
            s2f_intercept = st.number_input("S2F Intercept", min_value=0.0, max_value=100.0, value=float(data.get('s2f_intercept', 14.6)) if use_fetched_data else 14.6, help="S2F model intercept (default 14.6)")
            st.write(f"Fetched S2F Slope: {float(data.get('s2f_slope', 0.05)):.2f} (Default)")
            s2f_slope = st.number_input("S2F Slope", min_value=0.0, max_value=1.0, value=float(data.get('s2f_slope', 0.05)) if use_fetched_data else 0.05, help="S2F model slope (default 0.05)")
            st.write(f"Fetched Metcalfe Coefficient: {float(data.get('metcalfe_coeff', 0.0001)):.4f} (Default, adjust to 0.001 for higher valuation)")
            metcalfe_coeff = st.number_input("Metcalfe Coefficient", min_value=0.0, max_value=0.01, value=float(data.get('metcalfe_coeff', 0.0001)) if use_fetched_data else 0.0001, help="Scaling factor for Metcalfe's Law (default 0.0001)")
        
        with st.expander("Sentiment Inputs"):
            st.write(f"Fetched Fear & Greed: {int(data.get('fear_greed', 50))} (Alternative.me, expect 40-60)")
            fear_greed = st.number_input("Fear & Greed Index (0-100)", min_value=0, max_value=100, value=int(data.get('fear_greed', 50)) if use_fetched_data else 50, help="0=Extreme Fear, 100=Extreme Greed (Alternative.me)")
            st.write(f"Fetched Social Volume: {float(data.get('social_volume', 10000.0)):.0f} (Default)")
            social_volume = st.number_input("Social Volume (Mentions/Day)", min_value=0.0, value=float(data.get('social_volume', 10000.0)) if use_fetched_data else 10000.0, help="Social media mentions (default 10,000)")
            st.write(f"Fetched Sentiment Score: {float(data.get('sentiment_score', 0.5)):.2f} (Default)")
            sentiment_score = st.number_input("Sentiment Score (-1 to 1)", min_value=-1.0, max_value=1.0, value=float(data.get('sentiment_score', 0.5)) if use_fetched_data else 0.5, help="Positive=Bullish, Negative=Bearish (default 0.5)")
        
        with st.expander("Macro Inputs"):
            st.write(f"Fetched US Inflation: {float(data.get('us_inflation', 3.0)):.2f}% (Web-scraped, expect 2.5-3.5%)")
            us_inflation = st.number_input("US Inflation Rate (%)", min_value=0.0, max_value=50.0, value=float(data.get('us_inflation', 3.0)) if use_fetched_data else 3.0, help="Annual US inflation rate (web-scraped)")
            st.write(f"Fetched Fed Rate: {float(data.get('fed_rate', 5.0)):.2f}% (Web-scraped, expect 4.5-5.0%)")
            fed_rate = st.number_input("Fed Interest Rate (%)", min_value=0.0, max_value=50.0, value=float(data.get('fed_rate', 5.0)) if use_fetched_data else 5.0, help="Federal Reserve interest rate (web-scraped)")
            st.write(f"Fetched S&P 500 Correlation: {float(data.get('sp_correlation', 0.5)):.2f} (Calculated, expect 0.4-0.6)")
            sp_correlation = st.number_input("S&P 500 Correlation (0-1)", min_value=0.0, max_value=1.0, value=float(data.get('sp_correlation', 0.5)) if use_fetched_data else 0.5, help="BTC-S&P 500 correlation (calculated)")
            st.write(f"Fetched Gold Price: ${float(data.get('gold_price', 2000.0)):.2f} (Yahoo Finance, expect $2,000-$2,500)")
            gold_price = st.number_input("Gold Price (USD/oz)", min_value=0.0, value=float(data.get('gold_price', 2000.0)) if use_fetched_data else 2000.0, help="Gold price for comparison (Yahoo Finance)")
        
        with st.expander("Technical Inputs"):
            st.write(f"Fetched RSI: {float(data.get('rsi', 50.0)):.2f} (Yahoo Finance, expect 40-60)")
            rsi = st.number_input("RSI (14-day)", min_value=0.0, max_value=100.0, value=float(data.get('rsi', 50.0)) if use_fetched_data else 50.0, help="Overbought >70, Oversold <30 (Yahoo Finance)")
            st.write(f"Fetched 50-Day MA: ${float(data.get('50_day_ma', 57000.0)):.2f} (Yahoo Finance, expect $55,000-$65,000)")
            ma_50 = st.number_input("50-Day MA", min_value=0.0, value=float(data.get('50_day_ma', 57000.0)) if use_fetched_data else 57000.0, help="50-day moving average (Yahoo Finance)")
            st.write(f"Fetched 200-Day MA: ${float(data.get('200_day_ma', 54000.0)):.2f} (Yahoo Finance, expect $50,000-$60,000)")
            ma_200 = st.number_input("200-Day MA", min_value=0.0, value=float(data.get('200_day_ma', 54000.0)) if use_fetched_data else 54000.0, help="200-day moving average (Yahoo Finance)")
        
        with st.expander("Monte Carlo Settings"):
            st.write(f"Fetched Monte Carlo Runs: {int(data.get('monte_carlo_runs', 1000))} (Default)")
            monte_carlo_runs = st.number_input("Number of Runs", min_value=100, max_value=2000, value=int(data.get('monte_carlo_runs', 1000)) if use_fetched_data else 1000, help="100-2000 runs (default 1000)")
            st.write(f"Fetched Volatility Adjustment: {float(data.get('volatility_adj', 30.0)):.2f}% (Default)")
            volatility_adj = st.number_input("Volatility Adjustment Range (±%)", min_value=0.0, max_value=50.0, value=float(data.get('volatility_adj', 30.0)) if use_fetched_data else 30.0, help="Volatility variation (default 30%)")
            st.write(f"Fetched Growth Adjustment: {float(data.get('growth_adj', 20.0)):.2f}% (Default)")
            growth_adj = st.number_input("Growth Adjustment Range (±%)", min_value=0.0, max_value=50.0, value=float(data.get('growth_adj', 20.0)) if use_fetched_data else 20.0, help="Growth rate variation (default 20%)")
        
        st.write(f"Fetched Beta: {float(data.get('beta', 1.5)):.2f} (Default)")
        beta = st.number_input("Beta (vs. Market)", min_value=0.0, value=float(data.get('beta', 1.5)) if use_fetched_data else 1.5, help="BTC's market risk vs S&P 500 (default 1.5)")
        ...
        if calculate:
            with st.spinner("Calculating valuation..."):
                inputs = {
                    ...
                    '50_day_ma': ma_50,
                    '200_day_ma': ma_200,
                    'desired_return': float(data.get('desired_return', 15.0)),  # Ensure default
