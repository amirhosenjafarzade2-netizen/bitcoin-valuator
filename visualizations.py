...
def plot_heatmap(intrinsic_value, volatility, growth_rate):
    """
    Generate a heatmap for sensitivity analysis (intrinsic value vs volatility and growth rate).
    """
...
def plot_onchain_metrics(data, history):
    """
    Plot on-chain metrics over time.
    """
    if history.empty:
    ...
    df = history.copy()
    df['NVT'] = df['Close'] * data['circulating_supply'] / data['transaction_volume']
...
def plot_sentiment_analysis(data, history):
    """
    Plot sentiment metrics with price.
    """
    if history.empty:
